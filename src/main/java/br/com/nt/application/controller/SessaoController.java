...
public class SessaoController {
	...
	public ResponseEntity adicionarSessao (@Valid @RequestBody SessaoDto sessao) {
			try 
			{
				if(sessao.getDtValidade() == null ) sessao.setDtValidade( LocalDateTime.now().plusMinutes(1L));
				sessaoService.validaSessao(sessao);
				
				return ResponseEntity
							.status(HttpStatus.CREATED)
							.body(sessaoService.adicionar(sessao));
